...
let myLibrary = [
  { title: 'Woods', image: 'https://i.ibb.co/vPXFgCk/4.png', read: true },
  {
    title: 'The Whispering Throne',
    image: 'https://i.ibb.co/ncgGhVm/2.png',
    read: false,
  },
  {
    title: 'Hide And Seek',
    image: 'https://i.ibb.co/F05DKH1/3.jpg',
    read: true,
  },
];

function Book(title, image, read) {
  //construcor
  (this.title = title), (this.image = image), (this.read = read);
}

function curator(title, image, read) {
  //construct then push
  ...
  if (myLibrary.length > 0) {
    myLibrary.forEach((book, index) => {
      ...
      crossButton.addEventListener('click', function (event) {
        //deletes book array item and card
      ...
      readStatus.addEventListener('click', function () {
        ...
        tooltip.innerHTML =
          readStatus.style.color == 'red' ? 'Not read' : 'Read';
      });
...
const nameRegEx = '/^[a-z]{1}.[a-z0-9]{3,}$/';
